...
static  kbutton_t   in_cyclefiremode;
...
void IN_CycleFiremodeDown(const CCommand& args) { KeyDown(&in_cyclefiremode, args[1]); };
void IN_CycleFiremodeUp(const CCommand& args) { KeyUp(&in_cyclefiremode, args[1]); };
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_CYCLEFIREMODE, s_ClearInputState, &in_cyclefiremode, bResetState);
...
static ConCommand startcyclefiremode("+cyclefiremode", IN_CycleFiremodeDown);
static ConCommand endcyclefiremode("-cyclefiremode", IN_CycleFiremodeDown);
